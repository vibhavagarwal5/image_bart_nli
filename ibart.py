...
class BartEncoder(nn.Module):
    ...
    def __init__(self, config: BartConfig, embed_tokens):
        ...
        self.image_fc = nn.Linear(2048, config.d_model)

    def forward(self, input_ids, images=None, attention_mask=None, output_attentions=False, output_hidden_states=False):
        """
        Args:
        ...
        # T x B x C -> B x T x C
        encoder_states = [hidden_state.transpose(
            0, 1) for hidden_state in encoder_states]
        x = x.transpose(0, 1)
...
class BartDecoder(nn.Module):
    ...
    def forward(
        ...
        **unused,
        ...
        all_hidden_states = [hidden_state.transpose(
        ...
        if use_cache:
            next_cache = (
                (encoder_hidden_states, encoder_padding_mask), next_decoder_cache)
        else:
            next_cache = None
...
class SelfAttention(nn.Module):
    ...
    def forward(
        ...
        assert key_padding_mask is None or key_padding_mask.size()[
            :2] == (bsz, src_len,)
